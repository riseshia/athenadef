...
SQLの構文解析や検証は行わず、ファイルを文字列として読み取りAthenaに委任します。
...
**基本方針**: SQLの構文解析・検証は一切行わず、AWS Athenaに委任します。

- **構文チェック**: Athenaのクエリ実行時に自動的に検証
- **サポート機能**: AthenaがサポートするすべてのCREATE TABLE構文に対応
- **エラーハンドリング**: Athenaからのエラーメッセージをそのままユーザーに表示

この方針により：
- 複雑なSQL解析ライブラリが不要
- Athenaの機能更新に自動的に追従
- エラーメッセージがAthenaと一貫性を保持
- コードベースがシンプルで保守しやすい
